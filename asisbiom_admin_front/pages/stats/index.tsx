...
const Stats = () => {
  ...
  useMemo(() => {
    let alumnos: Alumno[] = [];
    useApi<Alumno[]>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/api/alumno`,
    }).then((res) => {
      alumnos = res.data;
    });
    useApi<number[]>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/stats/puntaje?positive-only=1`,
    }).then((res) => {
      console.log(res.data);
      setData(
        res.data.map((data, index) => {
          return { name: alumnos[index].nombreCompleto, puntaje: data };
        })
      );
      setSortedData(
        res.data
    ...
    useApi<number>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/stats/asistencias/porcentaje`,
    }).then((res) => setPorcentajeAsistencias(res.data));
    useApi<number>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/stats/puntualidad/porcentaje`,
    }).then((res) => setPorcentajePuntualidad(res.data));
    useApi<any>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/stats/distribucion-normal`,
    }).then((res) => setDistribucionNormal(res.data));
    useApi<number>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/stats/gini`,
    }).then((res) => setIndiceGini(res.data));
    useApi<number[]>({
      url: `${process.env.NEXT_PUBLIC_API_URL}/stats/lorenz`,
    }).then((res) => {
      const curvaData = res.data;
      const numAlumnos = alumnos.length;

      setCurva(
        curvaData.map((data, index) => {
          return {
            id: index,
            Alumno: alumnos[index].nombreCompleto,
            curva: data,
  ...
  return (
    <MainLayoutFixedHeight title="Estadísticas">
      ...
      <Paragraph>
        Las estadísticas de su institución se mostrarán aquí, para más
        información puede revisar la documentación oficial del proyecto
      </Paragraph>
      <Button
        target="_blank"
      ...
      <Card
        component={Paper}
        className="flex flex-col gap-2 p-4 w-full lg:w-1/2 sm:w-3/4"
      >
        ...
        {distribucionNormal && (
          <>
            ...
            <DistribucionNormal
              median={distribucionNormal.promedio}
              stddev={distribucionNormal.desviacionEstandar}
            />
            <div className="flex w-full justify-between">
...
export const DistribucionNormal: React.FC<{
  ...
  const data = Array.from({ length: lengthSlider }, (_, index) => {
    ...
    return {
      x: x,
      y:
        Math.pow(Math.E, -Math.pow(x - median, 2) / (2 * Math.pow(stddev, 2))) /
  ...
  return (
    <>
      ...
      <Slider
        ...
        aria-label="Volume"
        value={lengthSlider}
        onChange={(e: any) => setLengthSlider(e.target.value)}
      />
    </>
  );
};
