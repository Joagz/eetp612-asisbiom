package eetp612.com.ar.asisbiom.stats;

public class StatsConfigs {
    public static final String CANTIDAD_ALUMNOS_PRESENTES   = "CANTIDAD_ALUMNOS_PRESENTES";
    public static final String CANTIDAD_PERSONAL_PRESENTES  = "CANTIDAD_PERSONAL_PRESENTES";
    public static final String CANTIDAD_ALUMNOS             = "CANTIDAD_ALUMNOS";
    public static final String CANTIDAD_PERSONAL            = "CANTIDAD_PERSONAL";
    public static final String DIAS_HABILES                 = "DIAS_HABILES";
}
